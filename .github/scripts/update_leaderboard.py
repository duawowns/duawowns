...
# game -> (folder, sort, metric)
GAMES = {
    'boggle':  ('boggle-boggle', 'desc', 'score'),
    'othello': ('othello',       'desc', 'score'),
    'mine':    ('mine-friend',   'asc',  'time'),
    'sudoku':  ('sudoku',        'asc',  'time'),
}
...
if not re.match(r'^\[(boggle|othello|mine|sudoku)\]', title.strip()):
...
folder, sort, metric = GAMES[game]
val = data.get(metric, 0)
if not isinstance(val, (int, float)) or val < 0:
    skip(f'{metric} 값이 올바르지 않아요.')

name = str(data.get('name', '익명'))[:12].strip() or '익명'
name = re.sub(r'[\n\r`<>]', '', name)[:12] or '익명'

entry = {'name': name, metric: int(val)}
if metric == 'score':
    entry['combo'] = int(data.get('combo', 0) or 0)
    ...
    entry['diff'] = str(data.get('diff', ''))[:6]
    sort_key = lambda x: (x.get('time', 99999), x.get('name', ''))
    match = lambda e: e.get('time') == entry['time'] and e.get('diff', '') == entry['diff'] and e['name'] == entry['name']
    summary = f'**{name}** · **{int(val)}초** ({entry["diff"]})'
...
link = f'https://duawowns.github.io/duawowns/{folder}/'
